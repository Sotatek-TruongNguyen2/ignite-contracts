...
import {
  ...
  BEP20TokenImplementation__factory,
  BEP20USDT,
  BEP20USDT__factory,
  Errors__factory,
  FiatTokenV2_1,
...
describe("Ignition Pool With Vesting", () => {
  async function deployFactoryFixture() {
    ...
    pool = await new Pool__factory(
      {
        ...
        ["contracts/logics/VestingLogic.sol:VestingLogic"]:
    ...
    erc20TokenFactory = await new ERC20TokenFactory__factory(owner).deploy(
      erc20Sample.address
    );

    // Create purchase token
    await erc20TokenFactory
      .connect(owner)
      .createToken("PurchaseToken0", "PurchaseToken0", 6);
    const purchaseToken0Address = await erc20TokenFactory.currentToken();
    const purchaseToken0 = new ERC20Token__factory(owner).attach(
      purchaseToken0Address
    );

    await erc20TokenFactory
      .connect(owner)
      .createToken("PurchaseToken1", "PurchaseToken1", 8);
    const purchaseToken1Address = await erc20TokenFactory.currentToken();
    const purchaseToken1 = new ERC20Token__factory(owner).attach(
      purchaseToken1Address
    );

    await erc20TokenFactory
      .connect(owner)
      .createToken("PurchaseToken2", "PurchaseToken2", 18);
    const purchaseToken2Address = await erc20TokenFactory.currentToken();
    const purchaseToken2 = new ERC20Token__factory(owner).attach(
      purchaseToken2Address
    );

    await erc20TokenFactory
      .connect(owner)
      .createToken("PurchaseToken3", "PurchaseToken3", 6);
    const purchaseToken3Address = await erc20TokenFactory.currentToken();
    const purchaseToken3 = new ERC20Token__factory(owner).attach(
      purchaseToken3Address
    );

    await erc20TokenFactory
      .connect(owner)
      .createToken("PurchaseToken4", "PurchaseToken4", 6);
    const purchaseToken4Address = await erc20TokenFactory.currentToken();
    const purchaseToken4 = new ERC20Token__factory(owner).attach(
      purchaseToken4Address
    );

    USDC_ETH = await new FiatTokenV2_1__factory(owner).deploy();
    await USDC_ETH.initialize(
      "USD Coin",
      "USDC",
      "USDC",
      6,
    ...
    await USDC_BSC.initialize(
      "Binance USDC",
      "USDCBSC",
      18,
      parseUnits("100000000000000000000000", 30),
      true,
      owner.address
    );
    ...
    for (let i = 0; i < purchaseTokens.length; i++) {
      for (let j = 0; j < investors.length; j++) {
        if ((await purchaseTokens[i].symbol()) == "USDT_ETH") {
          ...
          await USDT_ETH.connect(owner).transfer(
            investors[j].address,
          ...
          await USDC_BSC.connect(owner).transfer(
            investors[j].address,
            parseUnits("1000000", "30")
          );
        } else if ((await purchaseTokens[i].symbol()) == "USDT") {
          // BSC
          await USDT_BSC.connect(owner).mint(parseUnits("1000000", "30"));
          await USDT_BSC.connect(owner).transfer(
            investors[j].address,
            parseUnits("1000000", "30")
          );
        } else {
          await purchaseTokens[i]
            .connect(owner)
            .mint(investors[j].address, parseUnits("1000000", "30"));
        }
      }
    }
    ...
    await erc20TokenFactory
      .connect(owner)
      .createToken("IDOToken0", "IDOToken0", 18);
    ...
    await erc20TokenFactory
      .connect(owner)
      .createToken("IDOToken1", "IDOToken1", 6);
    ...
    await erc20TokenFactory
      .connect(owner)
      .createToken("IDOToken2", "IDOToken2", 10);
    ...
    await erc20TokenFactory
      .connect(owner)
      .createToken("IDOToken3", "IDOToken3", 8);
    ...
    await erc20TokenFactory
      .connect(owner)
      .createToken("IDOToken4", "IDOToken4", 8);
    ...
    IDOTokens = [IDOToken0, IDOToken1, IDOToken2, IDOToken3, IDOToken4];

    await IDOTokens[0]
      .connect(owner)
      .mint(collaborator0.address, parseUnits("1000000", "30"));
    await IDOTokens[1]
      .connect(owner)
      .mint(collaborator1.address, parseUnits("1000000", "30"));
    ...
    poolInfoList = [
      {
        ...
        price: 0.08,
        TGEDate: BigNumber.from(Math.floor(Date.now() / 1000).toString()).add(
          30 * 24 * 60 * 60
        ),
  ...
  describe("Factory", () => {
    it("Should deploy pool factory and initialize pool implementation address successfully", async () => {
      const { factory, pool, vesting } = await loadFixture(
        deployFactoryFixture
      );
      expect(factory.address).to.be.a.properAddress;
      expect(await factory.poolImplementationAddress()).to.be.equal(
        pool.address
      );
      expect(await factory.vestingImplementationAddress()).to.be.equal(
        vesting.address
      );
    });

    it("Should grant, revoke admin successfully by admin", async () => {
      const { owner, admin1, admin2, admin3, factory } = await loadFixture(
        deployFactoryFixture
      );
      expect(
        await factory.hasRole(
          keccak256(solidityPack(["string"], ["OWNER_ROLE"])),
          owner.address
        )
      ).to.be.true;
      expect(
    ...
    it("Should set pool and vesting implementation address successfully by admin; revert set zero address; revert set pool and vesting implementation address not by admin", async () => {
      const { owner, admin1, admin2, factory, provider, errors, Events } =
      ...
      let randomPool = new ethers.Wallet(
        ethers.utils.randomBytes(32),
        provider
      );
      expect(
        await factory.connect(admin1).setPoolImplementation(randomPool.address)
      ).to.be.emit(factory, Events.Factory.UpdatePoolImplementation);
      expect(await factory.poolImplementationAddress()).to.be.equal(
        randomPool.address
      );
      ...
      ).to.be.revertedWith(await errors.ZERO_ADDRESS_NOT_VALID());
      await expect(
        factory.connect(admin2).setPoolImplementation(randomPool.address)
      ).to.be.revertedWith(await errors.CALLER_NOT_OWNER());

      randomPool = new ethers.Wallet(ethers.utils.randomBytes(32), provider);
      expect(
        await factory
          .connect(admin1)
          .setVestingImplementation(randomPool.address)
      ).to.be.emit(factory, Events.Factory.UpdateVestingImplementation);
      expect(await factory.vestingImplementationAddress()).to.be.equal(
        randomPool.address
      );
      ...
      await expect(
        factory
          .connect(owner)
          .setVestingImplementation(ethers.constants.AddressZero)
      ).to.be.revertedWith(await errors.ZERO_ADDRESS_NOT_VALID());
      await expect(
        factory.connect(admin2).setVestingImplementation(randomPool.address)
      ).to.be.revertedWith(await errors.CALLER_NOT_OWNER());
    });
    ...
    it("Should create pool successfully; revert if not valid pool info", async () => {
      ...
      expect(
        await factory
          .connect(collaborator0)
          .createPool(
            [poolInfo1.IDOToken, poolInfo1.purchaseToken],
            [
      ...
      await expect(
        factory
          .connect(owner)
          .createPool(
            [poolInfo4.IDOToken, poolInfo4.purchaseToken],
            [
      ...
      ).to.be.revertedWith(await errors.ZERO_ADDRESS_NOT_VALID());
      {
        ...
        ).to.be.revertedWith(await errors.INVALID_GALAXY_POOL_PROPORTION());
      }
      {
        let poolInfo4 = jsonCopy(poolInfoList[4]);
        ...
        ).to.be.revertedWith(await errors.INVALID_EARLY_ACCESS_PROPORTION());
      }
      {
        let poolInfo4 = jsonCopy(poolInfoList[4]);
        ...
        ).to.be.revertedWith(await errors.ZERO_AMOUNT_NOT_VALID());
      }
      {
        let poolInfo4 = jsonCopy(poolInfoList[4]);
        ...
        ).to.be.revertedWith(await errors.INVALID_TGE_PERCENTAGE());
      }
    });
  });

  describe("Buy with deployed IDO token and vesting with USDC", () => {
    async function deployPool0Fixture() {
      ...
      const poolMinimalByteCode =
        "0x3d602d80600a3d3981f3363d3d373d3d3d363d73" +
        pool.address.slice(2) +
        "5af43d82803e903d91602b57fd5bf3";
      const initCodeHash = solidityKeccak256(
        ["bytes"],
        [arrayify(poolMinimalByteCode)]
      );
      ...
      const maxPurchaseAmountForGalaxyPool_pool0 = BigNumber.from(
        (Number(poolInfo0.totalRaiseAmount.hex) *
          Number(poolInfo0.galaxyPoolProportion.hex)) /
          PERCENTAGE_DENOMINATOR
      );
      ...
      const buyWhiteList = [
        {
          ...
          userType: WHALE_HASH,
          maxPurchaseWhetherOrNotKYC: Number(
            poolInfo0.maxPurchaseAmountForKYCUser.hex
          ),
          maxPurchaseBaseOnAllocation: Math.floor(
            Number(
              (Number(maxPurchaseAmountForGalaxyPool_pool0) /
                allocationForGalaxyPool) *
                allocationList[0]
            )
          ),
        },
        ...
        },
        {
          ...
          userType: WHALE_HASH,
          maxPurchaseWhetherOrNotKYC: Number(
            poolInfo0.maxPurchaseAmountForNotKYCUser.hex
          ),
          maxPurchaseBaseOnAllocation: Math.floor(
            Number(
              (Number(maxPurchaseAmountForGalaxyPool_pool0) /
      ...
      const leafNodes = buyWhiteList.map((obj) => {
        let leafNode = solidityPack(
          ...
          [
            obj.candidate,
            obj.userType,
            obj.maxPurchaseWhetherOrNotKYC,
            obj.maxPurchaseBaseOnAllocation,
          ]
        );
      ...
      function getProof(leafInfo: {
        candidate: string;
        userType: string;
        maxPurchaseWhetherOrNotKYC: number;
        maxPurchaseBaseOnAllocation: number;
      }) {
        const leaf = solidityPack(
          ...
          [
            leafInfo.candidate,
            leafInfo.userType,
            leafInfo.maxPurchaseWhetherOrNotKYC,
            leafInfo.maxPurchaseBaseOnAllocation,
          ]
        );
    ...
    it("Should get info from pool after created", async () => {
      const { jsonCopy, pool0, poolInfoList, vesting0 } = await loadFixture(
        deployPool0Fixture
      );
      const poolInfo0 = jsonCopy(poolInfoList[0]);
      ...
      expect((await pool0.offeredCurrency()).rate).to.be.equal(
        Number(poolInfo0.rate.hex)
      );
      expect((await pool0.offeredCurrency()).decimal).to.be.equal(
        Number(poolInfo0.decimal.hex)
      );
      expect(await pool0.maxPurchaseAmountForKYCUser()).to.be.equal(
        Number(poolInfo0.maxPurchaseAmountForKYCUser.hex)
      );
      expect(await pool0.maxPurchaseAmountForNotKYCUser()).to.be.equal(
        Number(poolInfo0.maxPurchaseAmountForNotKYCUser.hex)
      );
      expect(await vesting0.TGEPercentage()).to.be.equal(
        Number(poolInfo0.TGEPercentage.hex)
      );
      expect(await pool0.galaxyParticipationFeePercentage()).to.be.equal(
        Number(poolInfo0.galaxyParticipationFeePercentage.hex)
      );
      expect(await pool0.crowdfundingParticipationFeePercentage()).to.be.equal(
        Number(poolInfo0.crowdfundingParticipationFeePercentage.hex)
      );
      expect(await pool0.galaxyPoolProportion()).to.be.equal(
        Number(poolInfo0.galaxyPoolProportion.hex)
      );
      expect(await pool0.earlyAccessProportion()).to.be.equal(
        Number(poolInfo0.earlyAccessProportion.hex)
      );
      expect(await pool0.totalRaiseAmount()).to.be.equal(
        Number(poolInfo0.totalRaiseAmount.hex)
      );
      expect(await pool0.whaleOpenTime()).to.be.equal(
        Number(poolInfo0.whaleOpenTime.hex)
      );
      expect(await pool0.whaleCloseTime()).to.be.equal(
        Number(poolInfo0.whaleDuration.hex) +
          Number(poolInfo0.whaleOpenTime.hex)
      );
      expect(await pool0.communityCloseTime()).to.be.equal(
        Number(poolInfo0.communityDuration.hex) +
          Number(poolInfo0.whaleOpenTime.hex) +
          Number(poolInfo0.whaleDuration.hex)
      );

      expect(await pool0.maxPurchaseAmountForGalaxyPool()).to.be.equal(
        Math.floor(
          (Number(poolInfo0.totalRaiseAmount.hex) *
            Number(poolInfo0.galaxyPoolProportion.hex)) /
            10000
      ...
      expect(await pool0.maxPurchaseAmountForEarlyAccess()).to.be.equal(
        Math.floor(
          (Number(poolInfo0.totalRaiseAmount.hex) *
            (10000 - Number(poolInfo0.galaxyPoolProportion.hex)) *
            Number(poolInfo0.earlyAccessProportion.hex)) /
            10000 /
            10000
        )
      );
      expect(await pool0.communityOpenTime()).to.be.equal(
        Number(poolInfo0.whaleOpenTime.hex) +
          Number(poolInfo0.whaleDuration.hex)
      );
    });
    ...
    it("Should set merkle tree root successfully by admin; revert without admin", async () => {
      const { pool0, buyRootHash, admin1, collaborator0, errors, Events } =
        await loadFixture(deployPool0Fixture);
      ...
      await expect(
        pool0.connect(collaborator0).setRoot(randomBuyRootHash)
      ).to.be.revertedWith(await errors.CALLER_NOT_ADMIN());
    });

    it("Should close pool by admin; revert without admin", async () => {
      const { pool0, admin1, Events } = await loadFixture(deployPool0Fixture);
      expect(await pool0.connect(admin1).cancelPool(true))
        .to.be.emit(pool0, Events.Pool.CancelPool)
        .withArgs(true);
    ...
    it("Should update time includes whale close time, crowdfunding close time, TGE date", async () => {
      ...
      expect(
        await pool0
          .connect(admin1)
          .updateTGEDate((await pool0.communityCloseTime()).add(100))
      ).to.be.emit(pool0, Events.Vesting.UpdateTGEDate);
      expect(
        await pool0
          .connect(admin1)
          .updateTime(
            await pool0.whaleCloseTime(),
            (await pool0.communityCloseTime()).add(50)
          )
      ).to.be.emit(pool0, Events.Pool.UpdateTime);
    });
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      await expect(
        pool0
          ...
          .buyTokenInGalaxyPool(
            proof0,
            100000,
            leafInfo0.maxPurchaseBaseOnAllocation
          )
      ).to.be.revertedWith(await errors.TIME_OUT_TO_BUY_IDO_TOKEN());
      ...
      ).to.be.revertedWith(await errors.NOT_ENOUGH_ALLOWANCE());
      // await purchaseTokens[0].connect(investors[0]).approve(pool0.address, ethers.constants.MaxUint256)
      await purchaseTokens[0]
        .connect(investors[0])
        .approve(
          pool0.address,
          BigNumber.from(leafInfo0.maxPurchaseBaseOnAllocation).add(1000)
        );
      expect(
        await pool0
          .connect(investors[0])
      ...
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      expect(await pool0.purchasedAmount()).to.be.equal(
        Number(parseUnits("90", 6))
      );
      expect(
        (await pool0.userPurchasedAmount(investors[0].address)).principal
      ).to.be.equal(Number(parseUnits("90", 6)));
      ...
      await purchaseTokens[0]
        .connect(investors[1])
        .approve(pool0.address, ethers.constants.MaxUint256);
      ...
      await expect(
        pool0
          ...
          .buyTokenInGalaxyPool(
            proof1,
            parseUnits("11000", 6),
            leafInfo1.maxPurchaseBaseOnAllocation
          )
      ).to.be.revertedWith(
        await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_KYC_USER()
      );
      ...
      expect(
        await pool0
          ...
          .buyTokenInGalaxyPool(
            proof4,
            parseUnits("870", 6),
            leafInfo4.maxPurchaseBaseOnAllocation
          )
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      ...
      await expect(
        ...
        await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_NOT_KYC_USER()
      );
      ...
      await expect(
        pool0
          ...
          .buyTokenInGalaxyPool(
            proof8,
            10,
            leafInfo8.maxPurchaseBaseOnAllocation
          )
      ).to.be.revertedWith(await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_USER());
      await expect(
        pool0.connect(investors[5]).buyTokenInGalaxyPool(proof8, 10, 10)
      ).to.be.revertedWith(await errors.NOT_IN_WHALE_LIST());
      ...
      const balanceOfInvestor0BeforeBuyToken =
        await purchaseTokens[0].balanceOf(investors[0].address);
      const balanceOfPool0BeforeBuyToken = await purchaseTokens[0].balanceOf(
        pool0.address
      );
      expect(
        await pool0
          .connect(investors[0])
          .buyTokenInCrowdfundingPool(proof0EA, parseUnits("100", 6))
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      const balanceOfInvestor0AfterBuyToken = await purchaseTokens[0].balanceOf(
        investors[0].address
      );
      const balanceOfPool0AfterBuyToken = await purchaseTokens[0].balanceOf(
        pool0.address
      );
      expect(
        balanceOfInvestor0AfterBuyToken.sub(balanceOfInvestor0BeforeBuyToken)
      ).to.be.equal(parseUnits("-110", 6));
      expect(
        balanceOfPool0AfterBuyToken.sub(balanceOfPool0BeforeBuyToken)
      ).to.be.equal(parseUnits("110", 6));
      ...
      expect(
        await pool0
          .connect(investors[0])
          .buyTokenInCrowdfundingPool(proof0EA, parseUnits("9800", 6))
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      ...
      expect(
        await pool0
          .connect(investors[3])
          .buyTokenInCrowdfundingPool(proof7EA, parseUnits("10", 6))
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      ...
      await expect(
        pool0
          .connect(investors[5])
          .buyTokenInCrowdfundingPool(proof8, parseUnits("10", 6))
      ).to.be.revertedWith(await errors.NOT_IN_WHALE_LIST());
      ...
      expect(
        await pool0
          .connect(investors[1])
          .buyTokenInCrowdfundingPool(proof10NU, parseUnits("100", 6))
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      ...
      await expect(
        pool0
          .connect(investors[0])
          .buyTokenInCrowdfundingPool(proof9NU, parseUnits("100", 6))
      ).to.be.revertedWith(
        await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_KYC_USER()
      );
      ...
      expect(
        await pool0
          .connect(investors[6])
          .buyTokenInCrowdfundingPool(proof17, parseUnits("100", 6))
      ).to.be.emit(pool0, Events.Pool.BuyToken);
      ...
      await expect(
        pool0
          .connect(investors[6])
          .buyTokenInCrowdfundingPool(proof17, parseUnits("10", 6))
      ).to.be.revertedWith(await errors.TIME_OUT_TO_BUY_IDO_TOKEN());

      // vesting
      // check vesting info
      expect(
        (await vesting0.vestingAmountInfo(investors[0].address)).totalAmount
      ).to.be.equal(parseUnits("124875", await IDOTokens[0].decimals())); // 1125 + 1250 + 122500 = 124875
      expect(
        (await vesting0.vestingAmountInfo(investors[1].address)).totalAmount
      ).to.be.equal(parseUnits("1250", await IDOTokens[0].decimals())); // 1250
      expect(
        (await vesting0.vestingAmountInfo(investors[2].address)).totalAmount
      ).to.be.equal(parseUnits("12125", await IDOTokens[0].decimals())); // 10875 + 1250 = 12125
      expect(
        (await vesting0.vestingAmountInfo(investors[3].address)).totalAmount
      ).to.be.equal(parseUnits("125", await IDOTokens[0].decimals())); // 125
      expect(
        (await vesting0.vestingAmountInfo(investors[4].address)).totalAmount
      ).to.be.equal(parseUnits("0", await IDOTokens[0].decimals()));
      expect(
        (await vesting0.vestingAmountInfo(investors[5].address)).totalAmount
      ).to.be.equal(parseUnits("0", await IDOTokens[0].decimals()));
      expect(
        (await vesting0.vestingAmountInfo(investors[6].address)).totalAmount
      ).to.be.equal(parseUnits("1250", await IDOTokens[0].decimals())); // 1250
      expect(
        (await vesting0.vestingAmountInfo(investors[7].address)).totalAmount
      ).to.be.equal(parseUnits("0", await IDOTokens[0].decimals()));
      ...
      await IDOTokens[0]
        ...
        .mint(
          collaborator0.address,
          await pool0.getIDOTokenAmountByOfferedCurrency(
            await pool0.totalRaiseAmount()
          )
        );
      await IDOTokens[0]
        .connect(collaborator0)
      ...
      expect(
        await pool0
          .connect(collaborator0)
          .fundIDOToken(IDOTokens[0].address, "0x")
      ).to.be.emit(pool0, Events.Pool.FundIDOToken);
      expect(await IDOTokens[0].balanceOf(vesting0.address)).to.be.equal(
        await pool0.getIDOTokenAmountByOfferedCurrency(
          await pool0.totalRaiseAmount()
        )
      );

      expect(await purchaseTokens[0].balanceOf(pool0.address)).to.be.equal(
        parseUnits("12181", await purchaseTokens[0].decimals())
      ); // 11170 + 1011 = 12181
      expect(await pool0.participationFeeAmount()).to.be.equal(
        parseUnits("1011", await purchaseTokens[0].decimals())
      ); // 1011
      ...
      expect(await pool0.connect(owner).claimParticipationFee(owner.address))
        .to.changeTokenBalance(
          purchaseTokens[0],
          owner.address,
          parseUnits("1011", await purchaseTokens[0].decimals())
        )
        .to.be.emit(pool0, Events.Pool.ClaimParticipationFee);
      const tokenFeeAmountWithDecimal =
        (11170 * (await pool0.tokenFeePercentage()) * 10 ** 6) / 10000;
      expect(await pool0.connect(owner).claimTokenFee(owner.address))
        .to.changeTokenBalance(
          purchaseTokens[0],
          owner.address,
          tokenFeeAmountWithDecimal
        )
        .to.be.emit(pool0, Events.Pool.ClaimTokenFee);
      const profitAmountWithDecimal =
        (11170 * (10000 - (await pool0.tokenFeePercentage())) * 10 ** 6) /
        10000;

      // claim profit fee by collaborator and IDO token by investor
      expect(
        await pool0.connect(collaborator0).claimFund(collaborator0.address)
      ).to.changeTokenBalance(
        purchaseTokens[0],
        collaborator0.address,
        (profitAmountWithDecimal * (await vesting0.TGEPercentage())) / 10000
      );
      await expect(
        pool0.connect(collaborator0).claimFund(collaborator0.address)
      ).to.be.revertedWith(await errors.INVALID_CLAIMABLE_AMOUNT());
      expect(
        await vesting0.connect(investors[0]).claimIDOToken(investors[0].address)
      ).to.be.changeTokenBalance(
        IDOTokens[0],
        investors[0].address,
        parseUnits("124875", await IDOTokens[0].decimals())
          .mul(await vesting0.TGEPercentage())
          .div(10000)
      );
      await expect(
        vesting0.connect(investors[0]).claimIDOToken(collaborator0.address)
      ).to.be.revertedWith(await errors.ALREADY_CLAIM_TOTAL_AMOUNT());
      expect(
        await vesting0.getClaimableAmount(investors[0].address)
      ).to.be.equal(0);
      ...
      await expect(
        vesting0.connect(investors[0]).claimIDOToken(investors[0].address)
      ).to.be.rejectedWith(await errors.ALREADY_CLAIM_TOTAL_AMOUNT());
      expect(
        await vesting0.getClaimableAmount(investors[0].address)
      ).to.be.equal(0);
      expect(await pool0.getClaimableFundAmount()).to.be.equal(0);
      ...
      expect(await pool0.connect(owner).setClaimableStatus(false)).to.be.emit(
        pool0,
        Events.Vesting.SetClaimableStatus
      );
      await expect(
        vesting0.connect(investors[0]).claimIDOToken(investors[0].address)
      ).to.be.revertedWith(await errors.NOT_ALLOWED_TO_CLAIM_IDO_TOKEN());
      await expect(
        pool0.connect(collaborator0).claimFund(collaborator0.address)
      ).to.be.revertedWith(await errors.NOT_ALLOWED_TO_CLAIM_PURCHASE_TOKEN());
      ...
      expect(
        (
          await vesting0
            .connect(investors[0])
            .vestingAmountInfo(investors[0].address)
        ).claimedAmount
      ).to.be.equal(
        (
          await vesting0
      ...
      expect(
        await pool0
          .connect(collaborator0)
          .withdrawRedundantIDOToken(collaborator0.address)
      ).to.be.emit(pool0, Events.Vesting.WithdrawRedundantIDOToken);
    });
  });
  ...
  describe("Buy without IDO token and vesting", () => {
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      const signature = await buildFundSignature(
        ...
        IDOTokens[0].address,
        pool0.address,
        await IDOTokens[0].symbol(),
        await IDOTokens[0].decimals()
      );
      expect(
        await pool0
          .connect(collaborator0)
          .fundIDOToken(IDOTokens[0].address, signature)
      ).to.be.emit(pool0, Events.Pool.FundIDOToken);
      expect(await IDOTokens[0].balanceOf(vesting0.address)).to.be.equal(
        await pool0.getIDOTokenAmountByOfferedCurrency(
          await pool0.purchasedAmount()
        )
      );
  ...
  describe("Pool is cancelled when buying", () => {
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      expect(
        (await vesting0.vestingAmountInfo(investors[1].address)).totalAmount
      ).to.be.equal(parseUnits("0", await IDOTokens[0].decimals())); // 0
      expect(
        (await vesting0.vestingAmountInfo(investors[2].address)).totalAmount
      ).to.be.equal(parseUnits("12125", await IDOTokens[0].decimals())); // 10875 + 1250 = 12125
      expect(
      ...
      // fund IDO token by collaborator
      await expect(
        pool0.connect(collaborator0).fundIDOToken(IDOTokens[0].address, "0x")
      ).to.be.revertedWith("Pausable: paused");

      expect(await purchaseTokens[0].balanceOf(pool0.address)).to.be.equal(
        parseUnits("11961", await purchaseTokens[0].decimals())
      ); // 10970 + 991 = 11961
      expect(await pool0.participationFeeAmount()).to.be.equal(
        parseUnits("991", await purchaseTokens[0].decimals())
      ); // 991
      ...
      await expect(
        pool0.connect(owner).claimParticipationFee(owner.address)
      ).to.be.revertedWith("Pausable: paused");
      await expect(
        pool0.connect(owner).claimTokenFee(owner.address)
      ).to.be.revertedWith("Pausable: paused");
      ...
      expect(
        await pool0
          .connect(investors[0])
          .withdrawPurchasedAmount(investors[0].address)
      )
        .to.be.emit(pool0, Events.Pool.WithdrawPurchasedAmount)
        .to.changeTokenBalance(
          purchaseTokens[0],
          investors[0].address,
          parseUnits("124875", await IDOTokens[0].decimals())
  ...
  describe("Pool with deployed IDO token is not funded before TGE date", () => {
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      await expect(
        pool0.connect(owner).claimParticipationFee(owner.address)
      ).to.be.revertedWith(await errors.NOT_FUNDED());
      const tokenFeeAmountWithDecimal =
        (11170 * (await pool0.tokenFeePercentage()) * 10 ** 6) / 10000;
      ...
      await expect(
        pool0.connect(collaborator0).fundIDOToken(IDOTokens[0].address, "0x")
      ).to.be.revertedWith(await errors.NOT_ALLOWED_TO_DO_AFTER_TGE_DATE());
  ...
  describe("Pool without prefix IDO token is not funded before TEG date", () => {
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      const signature = await buildFundSignature(
        ...
        IDOTokens[0].address,
        pool0.address,
        solidityKeccak256(["string"], [await IDOTokens[0].symbol()]),
        await IDOTokens[0].decimals()
      );
      await expect(
        pool0
  ...
  describe("Buy with deployed IDO token and vesting, then pool is emergency cancelled", () => {
    ...
    it("Should get info from pool after created", async () => {
      const { VestingFrequency, jsonCopy, pool0, poolInfoList, vesting0 } =
        await loadFixture(deployPool0Fixture);
      const poolInfo0 = jsonCopy(poolInfoList[0]);
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      expect(await pool0.connect(owner).cancelPool(false)).to.be.emit(
        vesting0,
        Events.Vesting.SetEmergencyCancelled
      );
      await time.increase(14 * 24 * 60 * 60);
      ...
      await expect(
        vesting0.connect(investors[0]).claimIDOToken(investors[0].address)
      ).to.be.rejectedWith(await errors.NOT_ALLOWED_TO_CLAIM_IDO_TOKEN());
      ...
      await expect(
        ...
        await errors.NOT_ALLOWED_TO_DO_AFTER_EMERGENCY_CANCELLED()
      );
    });
  });

  describe("Buy with deployed IDO token with USDT", () => {
    async function deployPool1Fixture() {
      ...
      const pool1Address = ethers.utils.getCreate2Address(
        factory.address,
        salt,
        initCodeHash
      );

      const vesting1Address = ethers.utils.getContractAddress({
        from: factory.address,
        nonce: 2,
      });
      await factory
      ...
      expect(await pool1.vesting()).to.be.equal(vesting1Address);

      const vesting1 = new Vesting__factory(
        {
          ["contracts/logics/VestingLogic.sol:VestingLogic"]:
            vestingLogic.address,
        },
        owner
      ).attach(vesting1Address);
      ...
      const maxPurchaseAmountForGalaxyPool_pool0 = BigNumber.from(
        (Number(poolInfo1.totalRaiseAmount.hex) *
          Number(poolInfo1.galaxyPoolProportion.hex)) /
          PERCENTAGE_DENOMINATOR
      );
      ...
      const buyWhiteList = [
        {
          ...
          userType: WHALE_HASH,
          maxPurchaseWhetherOrNotKYC: Number(
            poolInfo1.maxPurchaseAmountForKYCUser.hex
          ),
          maxPurchaseBaseOnAllocation: Math.floor(
            Number(
              (Number(maxPurchaseAmountForGalaxyPool_pool0) /
        ...
        },
        {
          ...
          userType: WHALE_HASH,
          maxPurchaseWhetherOrNotKYC: Number(
            poolInfo1.maxPurchaseAmountForNotKYCUser.hex
          ),
          maxPurchaseBaseOnAllocation: Math.floor(
            Number(
              (Number(maxPurchaseAmountForGalaxyPool_pool0) /
    ...
    it("Should get info from pool after created", async () => {
      const { jsonCopy, pool1, poolInfoList, vesting1 } = await loadFixture(
        deployPool1Fixture
      );
      const poolInfo1 = jsonCopy(poolInfoList[1]);

      expect(await vesting1.IDOToken()).to.be.equal(poolInfo1.IDOToken);
      expect(await pool1.purchaseToken()).to.be.equal(poolInfo1.purchaseToken);
      expect((await pool1.offeredCurrency()).rate).to.be.equal(
        BigNumber.from(poolInfo1.rate.hex)
      );
      expect((await pool1.offeredCurrency()).decimal).to.be.equal(
        Number(poolInfo1.decimal.hex)
      );
      expect(await pool1.maxPurchaseAmountForKYCUser()).to.be.equal(
        Number(poolInfo1.maxPurchaseAmountForKYCUser.hex)
      );
      expect(await pool1.maxPurchaseAmountForNotKYCUser()).to.be.equal(
        Number(poolInfo1.maxPurchaseAmountForNotKYCUser.hex)
      );
      expect(await vesting1.TGEPercentage()).to.be.equal(
        Number(poolInfo1.TGEPercentage.hex)
      );
      expect(await pool1.galaxyParticipationFeePercentage()).to.be.equal(
        Number(poolInfo1.galaxyParticipationFeePercentage.hex)
      );
      expect(await pool1.crowdfundingParticipationFeePercentage()).to.be.equal(
        Number(poolInfo1.crowdfundingParticipationFeePercentage.hex)
      );
      expect(await pool1.galaxyPoolProportion()).to.be.equal(
        Number(poolInfo1.galaxyPoolProportion.hex)
      );
      expect(await pool1.earlyAccessProportion()).to.be.equal(
        Number(poolInfo1.earlyAccessProportion.hex)
      );
      expect(await pool1.totalRaiseAmount()).to.be.equal(
        Number(poolInfo1.totalRaiseAmount.hex)
      );
      expect(await pool1.whaleOpenTime()).to.be.equal(
        Number(poolInfo1.whaleOpenTime.hex)
      );
      expect(await pool1.whaleCloseTime()).to.be.equal(
        Number(poolInfo1.whaleDuration.hex) +
          Number(poolInfo1.whaleOpenTime.hex)
      );
      expect(await pool1.communityCloseTime()).to.be.equal(
        Number(poolInfo1.communityDuration.hex) +
          Number(poolInfo1.whaleOpenTime.hex) +
          Number(poolInfo1.whaleDuration.hex)
      );

      expect(await pool1.maxPurchaseAmountForGalaxyPool()).to.be.equal(
        Math.floor(
          (Number(poolInfo1.totalRaiseAmount.hex) *
            Number(poolInfo1.galaxyPoolProportion.hex)) /
            10000
      ...
      expect(await pool1.maxPurchaseAmountForEarlyAccess()).to.be.equal(
        Math.floor(
          (Number(poolInfo1.totalRaiseAmount.hex) *
            (10000 - Number(poolInfo1.galaxyPoolProportion.hex)) *
            Number(poolInfo1.earlyAccessProportion.hex)) /
            10000 /
            10000
        )
      );
      expect(await pool1.communityOpenTime()).to.be.equal(
        Number(poolInfo1.whaleOpenTime.hex) +
          Number(poolInfo1.whaleDuration.hex)
      );
    });
    ...
    it("Should set merkle tree root successfully by admin; revert without admin", async () => {
      const { pool1, buyRootHash, admin1, collaborator0, errors, Events } =
        await loadFixture(deployPool1Fixture);
      ...
      await expect(
        pool1.connect(collaborator0).setRoot(randomBuyRootHash)
      ).to.be.revertedWith(await errors.CALLER_NOT_ADMIN());
    });

    it("Should close pool by admin; revert without admin", async () => {
      const { pool1, admin1, Events } = await loadFixture(deployPool1Fixture);
      expect(await pool1.connect(admin1).cancelPool(true))
        .to.be.emit(pool1, Events.Pool.CancelPool)
        .withArgs(true);
    ...
    it("Should update time includes whale close time, crowdfunding close time, TGE date", async () => {
      ...
      expect(
        await pool1
          .connect(admin1)
          .updateTGEDate((await pool1.communityCloseTime()).add(100))
      ).to.be.emit(pool1, Events.Vesting.UpdateTGEDate);
      expect(
        await pool1
          .connect(admin1)
          .updateTime(
            await pool1.whaleCloseTime(),
            (await pool1.communityCloseTime()).add(50)
          )
      ).to.be.emit(pool1, Events.Pool.UpdateTime);
    });
    ...
    it("Should buy token in galaxy pool, early access and open time successfully by all types of user; revert out of time and vesting", async () => {
      ...
      ).to.be.emit(pool1, Events.Pool.BuyToken);
      expect(await pool1.purchasedAmount()).to.be.equal(
        Number(parseUnits("90", 6))
      );
      expect(
        (await pool1.userPurchasedAmount(investors[0].address)).principal
      ).to.be.equal(Number(parseUnits("90", 6)));
      ...
      await purchaseTokens[1]
        .connect(investors[1])
        .approve(pool1.address, ethers.constants.MaxUint256);
      // await purchaseTokens[1].connect(investors[1]).approve(pool1.address, BigNumber.from(leafInfo1.maxPurchaseBaseOnAllocation))
      // await expect(pool1.connect(investors[1]).buyTokenInGalaxyPool(proof1, parseUnits('11000', 6), leafInfo1.maxPurchaseBaseOnAllocation)).to.be.revertedWith(await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_KYC_USER())
      ...
      await purchaseTokens[1]
        .connect(investors[2])
        .approve(pool1.address, ethers.constants.MaxUint256);
      // await purchaseTokens[1].connect(investors[2]).approve(pool1.address, BigNumber.from(leafInfo4.maxPurchaseBaseOnAllocation))
      expect(
        await pool1
          .connect(investors[2])
          .buyTokenInGalaxyPool(
      ...
      // revert buy more than max purchase amount for Not KYC user in twice (investor2, WHALE, Not KYC user, galaxy pool)
      // await expect(pool1.connect(investors[2]).buyTokenInGalaxyPool(proof4, parseUnits('50', 6), leafInfo4.maxPurchaseBaseOnAllocation)).to.be.revertedWith(await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_NOT_KYC_USER())
      ...
      await expect(
        pool1.connect(investors[5]).buyTokenInGalaxyPool(proof8, 10, 10)
      ).to.be.revertedWith(await errors.NOT_IN_WHALE_LIST());
      ...
      // buy successfully (investor0, WHALE, KYC User, early access)
      // await purchaseTokens[1].connect(investors[0]).approve(pool1.address, parseUnits('1000000',20))
      const balanceOfInvestor0BeforeBuyToken =
        await purchaseTokens[1].balanceOf(investors[0].address);
      const balanceOfpool1BeforeBuyToken = await purchaseTokens[1].balanceOf(
        pool1.address
      );
      ...
      const balanceOfInvestor0AfterBuyToken = await purchaseTokens[1].balanceOf(
        investors[0].address
      );
      const balanceOfpool1AfterBuyToken = await purchaseTokens[1].balanceOf(
        pool1.address
      );
      // expect((balanceOfInvestor0AfterBuyToken).sub(balanceOfInvestor0BeforeBuyToken)).to.be.equal(parseUnits('-110', 6))
      // expect((balanceOfpool1AfterBuyToken).sub(balanceOfpool1BeforeBuyToken)).to.be.equal(parseUnits('110', 6))
      await purchaseTokens[1].connect(investors[0]).approve(pool1.address, 0);
      await purchaseTokens[1]
        .connect(investors[0])
        .approve(pool1.address, parseUnits("1127000", 6));
      // buy successfully upto max purchase of KYC in galaxy and after in early access (investor0, WHALE, KYC User, early access)
      expect(
        await pool1
          .connect(investors[0])
      ...
      await purchaseTokens[1]
        .connect(investors[3])
        .approve(pool1.address, ethers.constants.MaxUint256);
      // await purchaseTokens[1].connect(investors[3]).approve(pool1.address, parseUnits('10',20))
      expect(
        await pool1
          .connect(investors[3])
          .buyTokenInCrowdfundingPool(proof7EA, parseUnits("10", 6))
      ...
      // revert buy more than max purchase of KYC in galaxy, after in early access and lately in community pool (not early access) (investor0, WHALE, KYC User, community pool)
      // await expect(pool1.connect(investors[0]).buyTokenInCrowdfundingPool(proof9NU, parseUnits('100', 6))).to.be.revertedWith(await errors.EXCEED_MAX_PURCHASE_AMOUNT_FOR_KYC_USER())

      // buy successfully (investor6, NORMAL, Not KYC user, community pool)
      await purchaseTokens[1]
        .connect(investors[6])
        .approve(pool1.address, ethers.constants.MaxUint256);
      // await purchaseTokens[1].connect(investors[6]).approve(pool1.address, parseUnits('100',20));
      const leaf17 = buyWhiteList[17];
      const proof17 = getProof(leaf17);
